...
if bMemoria:
    if psutil.virtual_memory().total < 17005105152:  # 16GB
        print(
            "ATENÇÃO. Como este script tenta gerar a tabela primeiro na memória RAM, há um requisito minimo de 16GB. Por isso este script possivelmente dará erro."
        )
        sys.exit()
...
resp = input(
    f"Este script vai criar ou alterar a base {camDBrede}. Leva cerca de 1 ou 2hs. Deseja prosseguir (y,n)?"
)
...
def executaSequencia(camDB, sqlsequencia):
    ...
    if bMemoria:
        bckengine = sqlite3.connect(
            camDB, detect_types=sqlite3.PARSE_DECLTYPES, uri=True
        )
