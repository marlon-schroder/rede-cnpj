...
nome_modulo_relacionamento = (
    config.config["BASE"].get("modulo_relacionamento", "rede_sqlite_cnpj").strip()
)
...
try:  # define alguma atividade quando é chamado por /rede-cnpj/envia_json, função serve_envia_json_acao
    import rede_acao
except:
    pass
...
api_key_validas = [
    k.strip() for k in config.config["API"].get("api_keys", "").split(",")
]
...
try:
    ...
    gLock = (
        threading.Lock()
    )  # prevenir erros de requisições seguidas. No servidor faz o esperado colocando só um thread no rede.wsgi.ini
...
# @app.route("/")
# def raiz():
#     return redirect("/rede-cnpj/", code = 302)


@app.route("/rede-cnpj/", methods=["GET", "POST"])
@app.route("/rede-cnpj/grafico/<int:camada>/<cpfcnpj>")
@app.route("/rede-cnpj/grafico_no_servidor/<idArquivoServidor>")
...
def serve_html_pagina(cpfcnpj="", camada=0, idArquivoServidor=""):
    ...
    idArquivoServidor = (
        idArquivoServidor if idArquivoServidor else config.par.idArquivoServidor
    )
    ...
    if config.par.arquivoEntrada:
        ...
        if extensao in [".py", ".js"]:
            listaEntrada = open(
                config.par.arquivoEntrada, encoding=config.par.encodingArquivo
            ).read()
            ...
            df = pd.read_csv(
                ...
                sep=config.par.separador,
                dtype=str,
                header=None,
                keep_default_na=False,
                encoding=config.par.encodingArquivo,
            ...
            df = pd.read_excel(
                config.par.arquivoEntrada,
                sheet_name=config.par.excel_sheet_name,
                header=None,
                dtype=str,
                keep_default_na=False,
            )
            ...
            print(
                "arquivo em extensão não reconhecida, deve ser csv, txt ou json:"
                + config.par.arquivoEntrada
            )
            sys.exit(0)
        if extensao in [".csv", ".txt", ".xlsx", "xls"]:
            ...
            for linha in df.values:
                listaEntrada += (
                    "\t".join([i.replace("\t", " ") for i in linha]) + "\n"
                )  # isso deve ser um caso particular, tab dentro de valor troca para espaço.
            # print(listaEntrada)
            df = None
    elif (
        not cpfcnpj and not idArquivoServidor
    ):  # define cpfcnpj inicial, só para debugar.
        # cpfcnpj = config.par.cpfcnpjInicial
        mensagemInicial = (
            config.config["INICIO"]
            .get("mensagem_advertencia", "")
            .replace("\\n", "\n")
            .strip()
        )
        if mensagemInicial:
    ...
    if (
        request.method == "POST"
    ):  # atende chamadas javascript da função openWindowWithPost
        try:
            ...
            if isinstance(jsonaux, dict):
                ...
                listaJson = jsonaux.get(
                    "json", ""
                )  # temporário para manter compatibilidade reversa
                ...
                abort(
                    404,
                    "Situação não prevista, request POST diferente de dict ou de texto.",
                )
                return
        except:
    ...
    paramsInicial = {
        ...
        "bBaseFullTextSearch": 1
        if config.config["BASE"].get("base_receita_fulltext", "")
        else 0,
        ...
        "mobile": any(
            word in request.headers.get("User-Agent", "")
            for word in ["Mobile", "Opera Mini", "Android"]
        ),
        ...
        "itensFlag": gp[
            "itensFlag"
        ],  # ['situacao_fiscal', 'pep', 'ceis', 'cepim', 'cnep', 'acordo_leniência', 'ceaf', 'pgfn-fgts', 'pgfn-sida','pgfn-prev'];
    }
    # print(paramsInicial)
    config.par.idArquivoServidor = (
        ""  # apagar para a segunda chamada da url não dar o mesmo resultado.
    )
    config.par.arquivoEntrada = ""
...
# @app.route('/rede-cnpj/grafojson/cnpj/<int:camada>/<cpfcnpj>', methods=['GET','POST'])
@app.route(
    "/rede-cnpj/grafojson/<tipo>/<int:camada>/<cpfcnpj>",
    methods=[
        "POST",
    ],
)  # methods=['GET','POST'])
    ...
    try:
        ...
        with gLock:
            if not criterioCaminhos:
                noLig = rede_relacionamentos.camadasRede(
                    camada=abs(camada), listaIds=listaIds, grupo="", bjson=True
                )
            elif criterioCaminhos:
                noLig = rede_relacionamentos.camadasRede(
                    camada=abs(camada),
                    grupo=listaIds,
                    criterioCaminhos=criterioCaminhos,
                    bjson=True,
                )
            r = jsonify(noLig)
...
@app.route(
    "/rede-cnpj/grafojson/links/<int:camada>/<int:numeroItens>/<int:valorMinimo>/<int:valorMaximo>/<cpfcnpj>",
    methods=["GET", "POST"],
)
@limiter.limit(limiter_padrao)
def serve_rede_json_links(
    cpfcnpj="", camada=1, numeroItens=15, valorMinimo=0, valorMaximo=0
):
    camada = min(gp["camadaMaxima"], int(camada))
    ...
    try:
        ...
        with gLock:
            r = jsonify(
                rede_relacionamentos.camadaLink(
                    listaIds=listaIds,
                    camada=camada,
...
# @lru_cache #isto pode dar inconsistência com parametros via post??
@app.route(
    "/rede-cnpj/dadosjson/<cpfcnpj>", methods=["GET", "POST"]
)  # methods=['POST'])
...
def serve_dados_detalhes(cpfcnpj):
    ...
    try:
        if gUwsgiLock:
            uwsgi.lock()
        with gLock:
            r = rede_relacionamentos.jsonDados(
                [
                    idin,
                ]
            )
...
@app.route(
    "/rede-cnpj/consulta_cnpj/", methods=["GET", "POST"]
)  # precisa manter com / no final para manter compatibilidade com robots
@limiter.limit(limiter_dados)
def serve_dados_html():
    try:
        if gUwsgiLock:
            uwsgi.lock()
        with gLock:
            return rede_relacionamentos.dados_consulta_cnpj(
                request, render_template, gp["itensFlag"]
            )
    finally:
        if gUwsgiLock:
            uwsgi.unlock()
...
if config.config["API"].getboolean("api_cnpj", False):
    ...
    @app.route("/rede-cnpj/api/<tipo>/<cnpj>", methods=["GET", "POST"])
    @limiter.limit(limiter_dados)
    def serve_dados_api(tipo, cnpj):
        try:
            ...
            with gLock:
                return jsonify(
                    rede_relacionamentos.dados_api_cnpj(cnpj, gp["itensFlag"])
                )
        finally:
            if gUwsgiLock:
                uwsgi.unlock()
...
if config.config["API"].getboolean("api_caminhos", False):
    ...
    @app.route("/rede-cnpj/api/caminhos", methods=["GET", "POST"])
    @limiter.limit(limiter_dados)
    def serve_api_caminhos():
        try:
            ...
            with gLock:
                ...
                if (dados.get("api_key", "") not in api_key_validas) or not dados.get(
                    "api_key"
                ):
                ...
                return jsonify(
                    rede_relacionamentos.camadasRede(
                        camada=camada,
                        grupo=dados["grupo"],
                        listaIds=None,
                        criterioCaminhos=dados["criterioCaminhos"],
                    )
                )
        finally:
            if gUwsgiLock:
...
# @app.route('/rede-cnpj/static/<path:arquivopath>') #, methods=['GET'])
...
local_file_dir = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), config.par.pasta_arquivos
)


@app.route("/rede-cnpj/arquivos_json/<arquivopath>", methods=["GET", "POST", "DELETE"])
@limiter.limit(limiter_padrao)
def serve_arquivos_json(arquivopath):
    ...
    if extensao in (".json", ".csv", ".xlsx", ".pdf"):
        ...
        if (
            filename.startswith("_temporario") or request.method == "DELETE"
        ):  # se for temporário, apaga depois de copiar dados para stream
            ...
            if (
                filename
                not in [
                    "rede_cnpj_diagrama.json",
                ]
                and extensao == ".json"
            ):
                os.remove(caminho)
            return send_file(
                return_data, mimetype="application/json", download_name=arquivopath
            )
        else:
...
@app.route("/rede-cnpj/arquivos_json_upload/<nomeArquivo>", methods=["POST"])
@limiter.limit(limiter_padrao)
...
@app.route("/rede-cnpj/arquivo_upload/", methods=["POST"])
@limiter.limit(limiter_padrao)
def serve_arquivos_upload():
    if not config.par.bArquivosDownload:
        return jsonify(
            {
                "nomeArquivoServidor": "",
                "mensagem": "salvamento de arquivo não autorizado",
            }
        )
        # return Response("Solicitação não autorizada", status=400)
...
@app.route("/rede-cnpj/json_para_base/<comentario>", methods=["POST"])
@limiter.limit(limiter_padrao)
...
@app.route("/rede-cnpj/envia_json/<acao>", methods=["POST"])
@limiter.limit(limiter_padrao)
...
# @app.route('/rede-cnpj/arquivos_download/<path:arquivopath>') #, methods=['GET'])
...
@app.route("/rede-cnpj/dadosemarquivo/<formato>", methods=["POST"])
...
def serve_dadosEmArquivo(formato="xlsx"):
    ...
    try:
        ...
        with gLock:
            ...
            if formato == "xlsx":
                return send_file(
                    rede_relacionamentos.dadosParaExportar(dados),
                    download_name="rede_cnpj-"
                    + time.strftime("%Y-%m-%d_%Hh%Mm")
                    + ".xlsx",
                    as_attachment=True,
                )
            elif formato == "anx":
...
@app.route("/rede-cnpj/mapa", methods=["POST"])
@limiter.limit(limiter_padrao)
def serve_mapa():
    dados = request.form.get("data")
    try:
...
# @app.route('/rede-cnpj/formdownload.html', methods = ['GET','POST'])
# @limiter.limit(limiter_padrao)
...
# @app.route('/rede-cnpj/abrir_arquivo/', methods = ['POST'])
@app.route("/rede-cnpj/abrir_arquivo", methods=["POST"])
@limiter.limit(limiter_padrao)
...
def serve_abrirArquivoLocal():
    ...
    if not os.path.exists(nomeArquivo):
        if nomeSplit[0]:
            ...
            return jsonify(
                {
                    "retorno": False,
                    "mensagem": "Não foi localizado na pasta arquivos do projeto.",
                }
            )
    if (extensao in kExtensaoDeArquivosPermitidos) and os.path.exists(nomeArquivo):
        ...
        return jsonify(
            {"retorno": False, "mensagem": "Extensão de arquivo não autorizada,"}
        )
...
if bConsultaChaves:
    ...
    @app.route("/rede-cnpj/busca_google", methods=["GET", "POST"])
    ...
    async def serve_busca_google_chave():
        ...
        try:
            ...
            if texto_busca:
                ...
                r = await rede_google.json_google_chaves(
                    texto_busca, pagina, n_palavras_chave
                )
            elif link:
                if link.startswith("LI_"):
                    link = link[3:]
                    r = await rede_google.json_busca_palavras_urls(
                        [link], n_palavras_chave
                    )
                elif link.startswith("AR_"):
                    caminhoDoc = caminhoArquivoLocal(link[3:])
                    r = rede_google.json_busca_palavras_doc(
                        link, caminhoDoc, n_palavras_chave
                    )
            ...
            return jsonify(
                {
                    "no": [],
                    "ligacao": [],
                    "mensagem": "Servidor não foi configurado para esta ação-A",
                }
            )
            ...
            return jsonify(
                {
                    "no": [],
                    "ligacao": [],
                    "mensagem": "Servidor não foi configurado para extrair as chaves. Baixe o projeto no github e rode a Rede localmente com essa opção ativada.",
                }
            )
        if not texto_busca:
            return jsonify({"no": [], "ligacao": [], "mensagem": "Sem texto"})
        try:
            r = rede_google.json_google_chaves_sincrono(
                texto_busca, pagina, n_palavras_chave=0
            )
            return r
        except:
            return jsonify(
                {
                    "no": [],
                    "ligacao": [],
                    "mensagem": "Servidor não foi configurado para esta ação-C",
                }
            )
...
if __name__ == "__main__":
    ...
    if platform.system() == "Windows":
        webbrowser.open(
            f"http://127.0.0.1:{config.par.porta_flask}/rede", new=0, autoraise=True
        )
